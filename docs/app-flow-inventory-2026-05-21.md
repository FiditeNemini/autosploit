...
- Static scans proving no S/M/L profile selectors or `ModelProfile` code remain
  ...
  `/qa/audit-ledger` route combines proof counts, proof category counts/total/
  parity, live artifact counts, visual capture counts, missing/failed artifact
  counts, and checkpoint completeness counts/ratio plus the current gap count
  into one machine-readable audit rollup.
