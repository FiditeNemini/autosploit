...
- TestServer `/state`, `/messages`, `/results` smoke via
  ...
  `/qa/artifact-ledger.visualCaptureStatus` and `missingVisualCaptures`, so the
  aggregate preserves artifact evidence paths and pass/fail status instead of
  only artifact counts. It also carries
  ...
  structured/raw parser tool sets, state-key count, tool-flow proof count,
  proof list/file parity, route list/count, family list/count, state-key list,
  contract map/count,
  ...
  map/count/parity, agent-loop current mode, max-iteration guard, proof count,
