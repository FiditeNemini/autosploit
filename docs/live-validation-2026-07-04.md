...
- [ ] Real-Qwen repo/codebase supply-chain scenario remains PARTIAL for 27B. `scripts/real-qwen-repo-codebase-supply-chain-proof.py` now performs RAM/process preflight before model load, forces TurboQuant q4 attention-KV with prefix/paged/block-L2 cache, uses Qwen3/Qwen parsers, requests `tool_choice=required` for exact `<tool_call>` prompts, stops stalled streams, retries missing tool calls, and writes `docs/live-proofs/2026-07-06-real-qwen-repo-codebase-supply-chain-27b.json`. The latest bounded live 27B run loaded `/Users/eric/models/dealign.ai/Qwen3.6-27B-MXFP8-CRACK-MTP` after memory preflight PASS and improved from prose-only drift to real tool execution, but still failed the full repo workflow. Artifact evidence at `2026-07-05T23:35:31-0700`: `ok=false`, `error="StalledFinalAnswer: assistant stream made no observable progress for 45.0s"`, four recovery attempts, and partial tool sequence `run_shell -> trufflehog -> syft -> grype`; missing tools remain `osv_scanner` and `search_cve`, and final marker `REAL_QWEN_REPO_SUPPLY_CHAIN_FINAL` was not reached. This is not a pass; next work is stricter remaining-tool execution or app-side exact-call injection so real Qwen completes `osv_scanner -> search_cve` and then emits the final marker without forcing a fake report.
