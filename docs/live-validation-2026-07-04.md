...
- Current release-app JSON framing proof was refreshed at `2026-07-05T16:50:59-0700` against the current package. `docs/live-proofs/2026-07-05-release-app-json-framing-live.json` reports `ok=true`, all `/state` and `/messages` responses parsed, `Content-Length` matched bytes, `Cache-Control: no-store`, no model process spawned, `appSha256=16821a81d9333eef49193ac0f60a9e3f8f806c7acb2a1cbad0c353f1bc355a51`, and `dmgSha256=8f6399c6a352b25550aca3a192fea233df308bcd703d5a64a63795042ad72b2c`. Distribution remains `BLOCKED` only by notarization/stapling, not by local package or JSON framing.
