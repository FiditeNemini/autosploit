...
- [ ] Refresh the full Computer Use Tools settings sweep and all-installed-tools smoke for the expanded 44-tool registry after installing or explicitly classifying `semgrep` and `bandit` on the current machine. Source wiring and local fixture proof are PASS; current visible 42-tool UI artifacts are historical and should not be used as current 44-tool inventory proof.
