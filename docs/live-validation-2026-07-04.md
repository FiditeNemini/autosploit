...
- Goal-audit matrix-count future-gate fix: `scripts/goal-requirement-audit-proof.py` now derives accepted matrix status counts from current matrix rows, so an eventual `26` PASS / `0` PARTIAL / `0` BLOCKED matrix is not rejected by the goal audit. Current artifacts still record the real current `24` PASS / `1` PARTIAL / `1` BLOCKED evidence state.
...
- Artifact `docs/live-proofs/2026-07-04-release-engine-stale-cleanup.json` reports `ok=true`, `foundCount=2`, `remainingCount=0`, `bundledLaunchCleanup=PASS`, `pidFileServerCleanup=PASS`, `bundledEnginePath=PASS`, `bundledRuntimeSelected=PASS`, and `modelLoaded=NO`.
...
- [x] Add/prove a multi-scenario autonomous pentest workflow matrix instead of relying on one-off tool checks. `scripts/repo-codebase-supply-chain-scenario-proof.py` now launches the current app in test mode, widens `toolSchemaMaxTools` to `64`, drives a local mock model through `/send`, creates a throwaway vulnerable repo fixture, installs deterministic local scanner shims into the app tool path, and proves the ordered sequence `run_shell -> trufflehog -> syft -> grype -> osv_scanner -> search_cve`. Artifact `docs/live-proofs/2026-07-06-repo-codebase-supply-chain-scenario.json` reports `status=PASS` for model tool-schema receipt, verbose chat tool transcript, ordered tool sequence, post-tool assistant continuation, repo surface evidence, secret/SBOM/dependency evidence, raw result ingestion, Supply lifecycle state, terminal transcripts, and report generation. The proof also exposed and fixed the `/tab`/`/qa/manual-tab-switch` mixed-case `supplyChain` API comparison. `scripts/autonomous-scenario-matrix-proof.py` produced `docs/live-proofs/2026-07-06-autonomous-scenario-matrix.json` with `4/4` PASS rows for local loopback webserver real-tools, Qwen phase-chain safe exploit/post validation, repo/codebase supply-chain, and report generation from evidence. The matrix aggregates older real-Qwen 27B/35B loopback/phase artifacts; it does not claim those Qwen rows were freshly rerun in this step.
...
- [ ] Real-Qwen repo/codebase supply-chain scenario remains PARTIAL for 27B. `scripts/real-qwen-repo-codebase-supply-chain-proof.py` now performs RAM/process preflight before model load, forces TurboQuant q4 attention-KV with prefix/paged/block-L2 cache, uses Qwen3/Qwen parsers, requests `tool_choice=required` for exact `<tool_call>` prompts, can set a one-shot function-specific `tool_choice` for missing-tool retries, finalizes after each specific-tool retry, stops stalled streams, and caps repeated failures for the same requested tool. The latest default-stall live 27B run loaded `/Users/eric/models/dealign.ai/Qwen3.6-27B-MXFP8-CRACK-MTP` after memory preflight PASS and produced real tool execution, but still failed the full repo workflow. Artifact `docs/live-proofs/2026-07-06-real-qwen-repo-codebase-supply-chain-27b.json` at `2026-07-06T00:07:46-0700`: `ok=false`, `error="StalledFinalAnswer: assistant stream made no observable progress for 45.0s"`, partial tool sequence `run_shell -> trufflehog -> syft`, premature `REAL_QWEN_REPO_SUPPLY_CHAIN_FINAL` text after `syft`, and then two function-specific `grype` retries that produced no `grype` tool call. Missing tools remain `grype`, `osv_scanner`, and `search_cve`. This is not a pass; next work is fixing model/tool parser behavior for specific `grype`/scanner retry turns without app-side fake tool execution.
- [ ] Remaining release gate: configure notary credentials, notarize, staple, rerun release/public-truth/notarization proofs, then rerun the goal audit. Completion claims remain blocked until this passes.
