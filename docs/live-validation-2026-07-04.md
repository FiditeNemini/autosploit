...
- Current objective open-blockers proof: BLOCKED overall, no model load. `scripts/objective-open-blockers-proof.py` wrote `docs/live-proofs/2026-07-05-objective-open-blockers-current.json` from the current PASS/PARTIAL/BLOCKED matrix and goal audit. It records `24` PASS, `2` PARTIAL, and `1` BLOCKED matrix rows, keeps `objectiveComplete=false` and `completionClaimAllowed=false`, and names exactly three remaining gates: `Full-context-length stress` needs true near-max final assistant output plus post-generation cache-write proof above the 192k proven safe ceiling, `Independent natural-language scenario tool selection` needs real 27B/35B local-fixture proofs without exact tool-call blocks, and `Release/distribution readiness` remains blocked by missing notary credentials, missing stapled app/DMG tickets, Gatekeeper rejection, and unpublished current-hash release assets.
...
- Open-blocker future-gate fix: `scripts/objective-open-blockers-proof.py` now derives open blocker rows from current matrix row status instead of permanently requiring the release row to be blocked. Current artifacts still list release distribution as BLOCKED, but after release evidence is updated to PASS the open-blocker artifact can shrink to the remaining long-context and independent-tool-choice PARTIAL rows.
- Matrix future-gate fix: `scripts/pass-partial-blocked-matrix-proof.py` now derives accepted status counts from current matrix rows instead of permanently validating only the current `24` PASS / `2` PARTIAL / `1` BLOCKED snapshot. Current artifacts record `24/2/1`, but a notarized release row can move the matrix to `25` PASS / `2` PARTIAL / `0` BLOCKED without the matrix proof rejecting the transition.
...
- Goal-audit matrix-count future-gate fix: `scripts/goal-requirement-audit-proof.py` now derives accepted matrix status counts from current matrix rows, so an eventual `27` PASS / `0` PARTIAL / `0` BLOCKED matrix is not rejected by the goal audit. Current artifacts record the real current `24` PASS / `2` PARTIAL / `1` BLOCKED evidence state.
- Independent natural tool-schema profile proof: PASS for schema filtering and execution blocking, still PARTIAL for real-model scenario completion. `scripts/tool-schema-profile-exclusion-proof.py` launches the current app in testing mode with a mock engine, applies `toolSchemaExcludedTools=["run_shell"]`, verifies `/qa/tool-catalog` omits `run_shell`, verifies `/state.chat.toolSchemaExcludedTools` records the exclusion, then feeds a mock model response that still attempts `run_shell`. Artifact `docs/live-proofs/2026-07-06-tool-schema-profile-exclusion.json` reports PASS for catalog exclusion, state recording, model attempt capture, blocked execution, and no `SHOULD_NOT_RUN` shell marker. This proves the app can present a scenario-focused tool schema and refuse excluded tool calls if a model emits them anyway; it does not prove Qwen 27B/35B can finish the natural scenario.
- Independent natural empty-argument repair proof: PASS for local target repair, still PARTIAL for real-model scenario completion. The earlier real 27B natural artifact showed the model repeatedly saying it would pass the scoped target to `httpx`, while the parsed tool call reached the executor as empty arguments and produced an `echo '' | httpx` command. `scripts/tool-argument-repair-proof.py` reproduces that failure with a mock model that emits `httpx` plus `{}` arguments, a prompt containing only a loopback target, and a deterministic fake `httpx`. After the fix, artifact `docs/live-proofs/2026-07-06-tool-argument-repair.json` reports PASS for model empty-argument emission, visible repair notice, command target use, fake `httpx` receiving the repaired target, no empty `httpx` command, and mock-engine-only execution. Boundary: repair is restricted to local-safe URLs parsed from the scoped prompt; this removes the empty-argument loopback failure mode but does not prove 27B/35B will independently finish with final/report proof.
- Post-repair real 27B natural rerun: improved but still non-passing. `scripts/real-qwen-natural-tool-choice-proof.py` was rerun against Qwen3.6 27B MXFP8 MTP after the repair. The saved artifact `docs/live-proofs/2026-07-06-real-qwen-natural-tool-choice-27b.json` now records `toolSequence=["httpx","katana","nuclei","sqlmap","lookup_cve"]` with PASS for `httpProbeEvidence`, `sqlInjectionProof`, `rawResultEvidence`, `terminalTranscripts`, TurboQuant q4 KV, paged cache, prefix cache, block disk cache, hybrid async SSM rederive, and native D3 MTP. The run was manually stopped when free pages dropped to a low-water state during final generation, and it still reports `overall=FAIL` because final assistant continuation/report completion and a stable accepted natural sequence for 27B/35B remain missing.
...
- [x] Refresh autonomous scenario matrix/catalog/artifact ledgers after the real-Qwen 27B/35B webserver SQLi, repo/codebase, and codebase static-analysis scenario passes. `docs/live-proofs/2026-07-06-autonomous-scenario-matrix.json` reports `status=PASS` across 6 rows, marks `webserver_auth_sqli_report_chain` as `modelRequired=true` with cache proof from both real-Qwen web artifacts, includes the new `codebase_static_to_patch_review_chain` row, `docs/live-proofs/2026-07-06-autonomous-scenario-catalog.json` keeps 6 local-only `READY_TO_RUN` scenarios, and `docs/live-proofs/2026-07-05-artifact-ledger-current.json` reports current live-proof failure-free status.
- [x] Make the autonomous scenario evidence boundary machine-readable. `scripts/autonomous-scenario-matrix-proof.py` now emits `toolChoiceMode`, `modelToolChoiceEvidence`, and `autonomyBoundary` for every row. The refreshed `docs/live-proofs/2026-07-06-autonomous-scenario-matrix.json` still reports `6` PASS rows, now folds the real-Qwen 27B/35B repo/codebase artifacts into `repo_codebase_supply_chain` with q4 TurboQuant KV, hybrid SSM, paged, and prefix cache proof, and labels the real-Qwen webserver, loopback, phase, and repo/codebase rows as `exact_tool_call_prompt_with_app_recovery`. Boundary: those rows prove real Qwen prompt/schema receipt, ordered tool execution, cache/MTP, verbose transcripts, and reports on local fixtures, but not independent natural-language tool selection.
- [x] Promote independent natural-language tool choice into the formal status gates. `docs/live-proofs/2026-07-04-pass-partial-blocked-matrix.json` now has `27` rows with `24` PASS, `2` PARTIAL, and `1` BLOCKED. The new `Independent natural-language scenario tool selection` row is PARTIAL and backed by the autonomous scenario matrix/catalog; `docs/live-proofs/2026-07-04-goal-requirement-audit.json` adds `independent_model_tool_choice`; `docs/live-proofs/2026-07-05-objective-open-blockers-current.json` now names three open rows: full-context stress, independent model tool choice, and release notarization/distribution readiness.
- [x] Attempt the first real-Qwen 27B independent natural-language webserver scenario and classify the result without hiding the failure. `scripts/real-qwen-natural-tool-choice-proof.py` launches the current app, a loopback SQLi fixture, Qwen3.6 27B MXFP8 MTP, TurboQuant q4 KV, prefix cache, paged cache, block L2 cache, and a natural-language objective with no exact tool-call blocks or forced function-specific retry. The current harness now excludes `run_shell` from the model-visible schema, includes deterministic local web-route tools such as `katana`, accepts `search_cve` or `lookup_cve` as the CVE context stage, and records cache/MTP proof plus schema receipt. The latest saved artifact `docs/live-proofs/2026-07-06-real-qwen-natural-tool-choice-27b.json` is still expected non-passing PARTIAL evidence, but it improved after the app-side local argument repair: the real model selected `httpx -> katana -> nuclei -> sqlmap -> lookup_cve`, preserved SQLi proof evidence, and passed raw/terminal evidence checks. The run was stopped for RAM safety before final continuation, so final marker/report proof and a stable 27B/35B PASS remain missing. Current blocker: produce stable 27B and 35B natural-objective passes through `surface -> probe -> prove -> evidence -> report` without exact tool-call blocks and without hitting the memory low-water stop.
...
- [ ] Remaining stress gate: full-context proof stays PARTIAL above the proven 192k safe ceiling. Current artifacts prove 192k completion, but 200k/224k/258k are guarded/refused or aborted by memory guard rather than completed with final output and cache writes.
