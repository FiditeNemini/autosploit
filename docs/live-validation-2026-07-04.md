...
- [x] Re-check current `release/ExploitBot.app` through live Computer Use after the latest GitHub checkpoint. `docs/live-proofs/2026-07-05-release-app-computer-use-current-manual-sweep.json` records CUA version `857` attached to `/Users/eric/exploitbot/release/ExploitBot.app` PID `66923`, every primary tab clicked from Recon through Stash with `manualTabSwitch` activity events, Settings Model/Runtime/Cache/Agents/CVE Database/Tools/Logs observed, 35B MXFP8 MTP selected with 14 scanned models and 6 supported entries, visible 27B/35B Qwen MTP `Use` controls, Runtime reasoning toggled on and applied then restored off and applied, Cache visibly kept `TurboQuant Q4` with prefix/prompt-L2/paged/block-L2 enabled, CVE Database showed `1644` total CVEs / `1631` CISA KEV and live local search results for `apache path traversal` including `CVE-2024-32113`, `CVE-2021-42013`, and `CVE-2021-41773`, Tools showed `42 installed` / `0 missing`, Logs empty-state was present, and no model engine was started during this UI sweep.
...
- [x] Refresh aggregate proof/artifact ledgers after the current report artifacts and classify the manual release UI sweep as an expected non-passing partial gate. `scripts/proof-ledger-proof.py` wrote `docs/live-proofs/2026-07-05-proof-ledger-current.json` at `2026-07-05T16:29:43-0700` with `proofCount=266`, route/file/category/tab parity PASS, and `modelInferenceStarted=NO`. `scripts/artifact-ledger-proof.py` wrote `docs/live-proofs/2026-07-05-artifact-ledger-current.json` at `2026-07-05T16:29:41-0700` with `liveProofCount=168`, `failedLiveProofCount=14`, `currentFailedLiveProofCount=0`, `currentLiveProofFailureFree=PASS`, and `modelInferenceStarted=NO`.
