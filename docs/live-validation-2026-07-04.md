...
- [x] Attempt the first real-Qwen 27B independent natural-language webserver scenario and classify the result without hiding the failure. `scripts/real-qwen-natural-tool-choice-proof.py` launches the current app, a loopback SQLi fixture, Qwen3.6 27B MXFP8 MTP, TurboQuant q4 KV, prefix cache, paged cache, block L2 cache, and a natural-language objective with no exact tool-call blocks or forced function-specific retry. The current harness now excludes `run_shell` from the model-visible schema, includes deterministic local web-route tools such as `katana`, and records cache/MTP proof plus schema receipt. The latest saved artifact `docs/live-proofs/2026-07-06-real-qwen-natural-tool-choice-27b.json` is still expected non-passing PARTIAL evidence: it shows capped natural web schemas, `run_shell` excluded, and TurboQuant/paged/prefix/cache/MTP proof, but the saved 27B run looped on `httpx` and did not produce the required ordered `httpx -> katana -> sqlmap -> search_cve` sequence, SQLi proof, final marker, or report proof. Current blocker: produce stable 27B and 35B natural-objective passes through `surface -> probe -> prove -> evidence -> report` without exact tool-call blocks.
