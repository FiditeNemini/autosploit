...
- Gowitness CLI mapping proof: PARTIAL. `ToolDefinitions.buildCliArgs(toolName:"gowitness", ...)` now emits `gowitness scan single --url ... --screenshot-path ...`, matching the source contract for the v3 command shape. `scripts/gowitness-cli-mapping-proof.py` writes `docs/live-proofs/2026-07-06-gowitness-cli-mapping.json`; the artifact reports `sourceUsesScanSingle=PASS` but `overall=PARTIAL` because `gowitness` is not installed on this machine, so `scan single --help` could not be live-verified. Boundary: no screenshot/target action was run.
