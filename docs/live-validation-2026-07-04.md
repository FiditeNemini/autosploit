...
- [x] Attempt the first real-Qwen 27B independent natural-language webserver scenario and classify the result without hiding the failure. `scripts/real-qwen-natural-tool-choice-proof.py` launches the current app, a loopback SQLi fixture, Qwen3.6 27B MXFP8 MTP, TurboQuant q4 KV, prefix cache, paged cache, block L2 cache, and a natural-language objective with no exact tool-call blocks or forced function-specific retry. Source and live evidence show the model received capped web schemas, `followAgent=false` kept the tab/schema pinned to web, and the model independently selected `httpx` followed by local route probes through `run_shell`; the run timed out before `nuclei`, `sqlmap`, CVE enrichment, final marker, or report proof. Artifact `docs/live-proofs/2026-07-06-real-qwen-natural-tool-choice-27b.json` is therefore expected non-passing PARTIAL evidence, not a PASS. Current blocker: build a natural-objective harness/schema policy that gets real 27B and 35B through `surface -> probe -> prove -> evidence -> report` without exact tool-call blocks.
