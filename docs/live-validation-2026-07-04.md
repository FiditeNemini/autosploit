...
- Fresh 17:21 no-model app QA route refresh for confirmed-finding reporting: PASS. `create_finding` is now exposed as an always-visible callback tool so the agent can record confirmed vulnerabilities with raw evidence and attack-chain steps before report generation. Source/proof surfaces were updated for callback inventory, result-mode metadata, catalog detail, tool-flow coverage, context-boundary coverage, app QA matrix counters, and i18n (`tool.create_finding` in en/ko/zh/es/ja). Live no-model route proofs passed: `scripts/tool-registry-coverage-proof.py`, `scripts/tool-flow-coverage-proof.py`, `scripts/tool-catalog-detail-proof.py`, `scripts/context-prompt-injection-boundary-proof.py`, `scripts/agent-flow-inventory-proof.py`, and `scripts/app-qa-matrix-smoke-proof.py`, refreshing `docs/live-proofs/2026-07-04-app-qa-matrix-smoke.json` at `2026-07-06T17:21:28-0700`.
- Fresh 17:09 release proof refresh on `HEAD=2078891`: PARTIAL/BLOCKED. `scripts/release-readiness-proof.py` rebuilt the local package and wrote `docs/live-proofs/2026-07-04-release-readiness.json` with `localPackageStatus=PASS`, app/DMG codesign PASS, bundled runtime PASS, app binary hash `ad079f5fa0388539ece9c5b35344055d1d36d529adad2fd45a23ba2a47bf5bb1`, DMG hash `8ef98f55f60997b2e1279930ee04f797086d0830526c0e6bb24b389047853aba`, and `distributionStatus=BLOCKED` because `notarizationGate=requires-notary-credentials`. `scripts/release-visible-smoke-proof.py` refreshed `docs/live-proofs/2026-07-04-release-visible-smoke.json` and screenshot `docs/live-proofs/2026-07-04-release-visible-smoke.png` with `localDisplayStatus=PASS`, visible window PASS, main workspace PASS, screenshot PASS, and `noModelLoaded=PASS`. `scripts/notarization-preflight-proof.py` refreshed `docs/live-proofs/2026-07-04-notarization-preflight.json` with `nextAction=configure-notary-credentials-and-run-package-notarize`, no configured notary inputs, no stapled ticket on the app or DMG, and Gatekeeper still blocked.
- Fresh 17:09 public release truth refresh: PARTIAL/BLOCKED. `scripts/release-public-truth-proof.py` refreshed `docs/live-proofs/2026-07-05-release-public-truth.json` against GitHub for current local manifest hash `d354fafceccf4986f7a0a7ef15bb0e61c3dc3f2b6cc959fb9ed9627924ae73e9` and DMG hash `8ef98f55f60997b2e1279930ee04f797086d0830526c0e6bb24b389047853aba`. The repo and `v0.1.0-beta` release/DMG/manifest assets are visible, but the published DMG digest, published manifest digest, release target commit `afe3ece9c0ce06405a3ec5eaecf0e27bef079bf2`, and local `notarizationGate=requires-notary-credentials` keep `releaseClaimAllowed=false`.
- Current objective open-blockers proof: BLOCKED overall, no model load. `scripts/objective-open-blockers-proof.py` refreshed `docs/live-proofs/2026-07-05-objective-open-blockers-current.json` at `2026-07-06T17:09:59-0700` from the current PASS/PARTIAL/BLOCKED matrix and goal audit. The remaining open gates are `Full-context-length stress`, which needs true near-max final assistant output plus post-generation cache-write proof above the 192k proven safe ceiling, and `Release/distribution readiness`, which remains blocked by missing notary credentials, missing stapled app/DMG tickets, Gatekeeper rejection, and unpublished current-hash release assets.
...
- Impacket CLI mapping proof: PASS for current-machine command construction. `ToolDefinitions.buildCliArgs(toolName:"impacket", ...)` now invokes installed console scripts such as `secretsdump.py` directly instead of `python3 -m impacket.examples.*`. `scripts/impacket-cli-mapping-proof.py` writes `docs/live-proofs/2026-07-06-impacket-cli-mapping.json`; the live artifact reports source mapping PASS, `secretsdump.py` present at `/Users/eric/.local/bin/secretsdump.py`, `secretsdump.py -h` exit 0, and the old module path `python3 -m impacket.examples.secretsdump -h` exit 1 with `ModuleNotFoundError`. Boundary: help commands only; no impacket action was run against a target.
- Gowitness CLI mapping proof: PASS for current-machine help and safe loopback screenshot. `ToolDefinitions.buildCliArgs(toolName:"gowitness", ...)` emits `gowitness scan single --url ... --screenshot-path ... --screenshot-format png`, matching the source contract for the v3 command shape and pinning output to an extension that `ResultsStore` ingests. `scripts/gowitness-cli-mapping-proof.py` writes `docs/live-proofs/2026-07-06-gowitness-cli-mapping.json`; the live artifact reports `sourceUsesScanSingle=PASS`, binary `/Users/eric/.exploitbot/tools/gowitness`, old `gowitness single --help` rejected, new `gowitness scan single --help` accepted, and `safeLoopbackScreenshot=PASS` after capturing one PNG from a temporary `127.0.0.1` fixture titled `ExploitBot Gowitness Fixture`. Boundary: this proves CLI shape and local screenshot capture only; no external target was contacted.
- Nuclei installer path proof: PASS for app-managed Go install wiring. `ToolInstaller.swift` now installs ProjectDiscovery `nuclei` with `GOBIN="$HOME/.exploitbot/tools"` like the other app-managed Go tools. `scripts/nuclei-app-managed-install-proof.py` writes `docs/live-proofs/2026-07-06-nuclei-app-managed-install.json`; the live artifact reports `sourceUsesAppManagedGoBin=PASS`, binary `/Users/eric/.exploitbot/tools/nuclei`, `appManagedBinaryVersionRuns=PASS`, and Nuclei Engine `v3.11.0`. Boundary: this proves installer command shape plus current-machine app-managed binary/version only; no target scan was run.
...
- `docs/live-proofs/2026-07-06-impacket-cli-mapping.json`
- `docs/live-proofs/2026-07-06-gowitness-cli-mapping.json`
- `docs/live-proofs/2026-07-06-nuclei-app-managed-install.json`
