...
- [ ] Real-Qwen repo/codebase supply-chain scenario remains PARTIAL for 27B. New guarded harness `scripts/real-qwen-repo-codebase-supply-chain-proof.py` performs RAM/process preflight before model load, forces TurboQuant q4 attention-KV with prefix/paged/block-L2 cache, uses Qwen3/Qwen parsers, and writes `docs/live-proofs/2026-07-06-real-qwen-repo-codebase-supply-chain-27b.json`. The bounded live 27B run loaded `/Users/eric/models/dealign.ai/Qwen3.6-27B-MXFP8-CRACK-MTP` after memory preflight PASS and began the local repo workflow, but the model drifted into a phase narrative and stalled before the required full tool sequence/final marker. Artifact evidence: `ok=false`, `error="StalledFinalAnswer: assistant stream made no observable progress for 45.0s"`, `finalAnswerAttempts[0].status="stalled-final-answer"`, and partial tool sequence `run_shell -> syft -> grype -> osv_scanner`. This is not a pass; next work is prompt/loop hardening so real Qwen completes `run_shell -> trufflehog -> syft -> grype -> osv_scanner -> search_cve` and emits `REAL_QWEN_REPO_SUPPLY_CHAIN_FINAL` without forcing a fake final answer.
