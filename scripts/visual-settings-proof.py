...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-73"
...
def assert_settings_state(category: str) -> None:
    state = request("GET", "/state")
    qa = state.get("qaSettingsVisual") or {}
    if qa.get("category") != category:
        raise AssertionError(f"settings visual category mismatch: expected {category}, got {qa}")
    if state.get("healthStatus") != "running":
        raise AssertionError(f"expected seeded running engine health: {state}")
    ...
    for key, value in required.items():
        if engine.get(key) != value:
            raise AssertionError(f"engine config mismatch for {key}: expected {value}, got {engine}")
...
def run() -> None:
    ...
    try:
        ...
        if seeded.get("ok") is not True:
            raise AssertionError(f"settings visual state seed failed: {seeded}")

        captures = []
        for category, filename in [
            ("model", "settings-model-warning.png"),
            ("engine", "settings-engine-cache-status.png"),
            ("cache", "settings-cache-topology.png"),
        ]:
            switched = request("POST", "/qa/settings-category", category)
            if switched.get("ok") is not True:
                raise AssertionError(f"settings category switch failed for {category}: {switched}")
            assert_settings_state(category)
        ...
        manifest = {
            "ok": True,
            "captures": captures,
            "note": "Cropped macOS captures of Settings model unsupported warning, engine live cache runtime status, and cache topology controls.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-settings proof passed")
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-settings proof failed: {exc}", flush=True)
        raise SystemExit(1)
