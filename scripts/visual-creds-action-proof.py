...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-99"
...
def run() -> None:
    ...
    try:
        ...
        if state.get("activeTab") != "creds" or (state.get("credsAction") or {}).get("status") != "done":
            raise AssertionError(f"expected done creds action state: {state}")
        target = OUT_DIR / "creds-cracked-badges.png"
        capture(target)
        manifest = {
            "ok": True,
            "captures": [str(target.relative_to(ROOT))],
            "note": "Cropped macOS capture of Creds Cracking tab showing Start Crack done state and CRACKED result badges.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-creds-action proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-creds-action proof failed: {exc}", flush=True)
        raise SystemExit(1)
