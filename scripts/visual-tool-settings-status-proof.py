...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-109"
...
def run() -> None:
    ...
    try:
        ...
        if tool_settings.get("installedCount") != 2 or tool_settings.get("errorCount") != 1:
            raise AssertionError(f"expected seeded tool settings state: {state}")

        target = OUT_DIR / "tool-settings-status.png"
        capture(target)
        manifest = {
            "ok": True,
            "captures": [str(target.relative_to(ROOT))],
            "note": "Cropped macOS capture of Tools settings showing installed, missing, installing, error rows, and install log state.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-tool-settings-status proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-tool-settings-status proof failed: {exc}", flush=True)
        raise SystemExit(1)
