...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-90"
...
def assert_state() -> None:
    ...
    if state.get("activeTab") != "osint":
        raise AssertionError(f"expected OSINT tab: {state}")
    artifact = (state.get("osintArtifacts") or [{}])[0]
    if artifact.get("previewKind") != "image" or artifact.get("exists") is not True:
        raise AssertionError(f"expected image artifact preview state: {state}")
    if (state.get("qaVisualSubtabs") or {}).get("osint") != "Screenshots":
        raise AssertionError(f"expected Screenshots subtab: {state}")
...
def run() -> None:
    ...
    try:
        ...
        assert_state()
        target = OUT_DIR / "osint-screenshot-preview.png"
        capture(target)
        manifest = {
            "ok": True,
            "captures": [str(target.relative_to(ROOT))],
            "note": "Cropped macOS capture of OSINT Screenshots tab showing validated image artifact preview and lifecycle state.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-osint-screenshot proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-osint-screenshot proof failed: {exc}", flush=True)
        raise SystemExit(1)
