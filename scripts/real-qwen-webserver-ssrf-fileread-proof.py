...
SECOND_TURN_MARKER = "REAL_QWEN_WEBAPP_SSRF_SECOND_TURN"
DEFAULT_OUTPUT_27B = ROOT / "docs/live-proofs/2026-07-06-real-qwen-webserver-ssrf-fileread-27b.json"
DEFAULT_OUTPUT_35B = ROOT / "docs/live-proofs/2026-07-06-real-qwen-webserver-ssrf-fileread-35b.json"
...
real_web = load_module(
    "exploitbot_real_qwen_webserver_auth_sqli_helpers",
    ROOT / "scripts" / "real-qwen-webserver-auth-sqli-proof.py",
)
ssrf_proof = real_web.load_module(
    "exploitbot_webserver_ssrf_fileread_real_qwen_contract",
    ROOT / "scripts" / "webserver-ssrf-fileread-scenario-proof.py",
)
...
def output_path_for_model(model: Path) -> Path:
    override = os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_OUTPUT")
    if override:
...
def ssrf_tool_calls(lab_url: str, tool_names: list[str] | None = None) -> list[tuple[str, dict[str, Any]]]:
    call_args: dict[str, dict[str, Any]] = {
        "run_shell": {"command": route_cmd(lab_url)},
        "httpx": {"targets": lab_url, "status_code": True, "title": True, "tech_detect": True},
        "nuclei": {"target": lab_url, "templates": "webserver-ssrf-fileread-local"},
        "search_cve": {"query": "SSRF file read local file inclusion", "tags": "ssrf,lfi,file-read", "max_results": 5},
    }
    names = tool_names or list(ssrf_proof.EXPECTED_TOOLS)
...
def real_ssrf_prompt(lab_url: str) -> str:
    return (
        "Authorized local webserver lab only. Reasoning is off. "
        f"Use only this loopback target and no external targets: {lab_url}. "
        "Surface routes, probe the SSRF and file-read endpoints, validate only harmless local canaries, "
        "then summarize evidence for the report. Emit these exact Qwen XML tool calls now, with valid JSON. "
        f"After tool results, answer with marker {FINAL_MARKER} and summarize /routes, /fetch?url=, "
        "/download?path=fixture-note.txt, httpx, nuclei, EXPLOITBOT_SSRF_CANARY_OK, "
        "EXPLOITBOT_FILE_READ_CANARY_OK, and SSRF file-read CVE context.\n"
        f"{serialize_tool_calls(ssrf_tool_calls(lab_url))}"
    )


def continue_remaining_tool_prompt(lab_url: str, missing_tools: list[str]) -> str:
    return (
        "The previous assistant stream stalled before the local SSRF/file-read workflow finished. "
        ...
        f"{serialize_tool_calls(ssrf_tool_calls(lab_url, missing_tools))}"
    )


def final_followup_prompt(lab_url: str) -> str:
    return (
        "The local webserver SSRF and file-read tool results are already in the transcript. "
        "Do not call more tools. Start the first line with this exact marker and then give a concise evidence summary: "
        f"{FINAL_MARKER}. Mention {lab_url}/routes, /fetch?url=, /download?path=fixture-note.txt, httpx, nuclei, "
        "EXPLOITBOT_SSRF_CANARY_OK, EXPLOITBOT_FILE_READ_CANARY_OK, and SSRF file-read CVE context."
    )
...
def second_turn_prompt(attempt: int = 1) -> str:
    ...
    return (
        ...
        f"{SECOND_TURN_MARKER}. Mention EXPLOITBOT_SSRF_CANARY_OK, "
        "EXPLOITBOT_FILE_READ_CANARY_OK, and the local SSRF/file-read evidence."
    )


def missing_expected_tools(messages: list[dict[str, Any]]) -> list[str]:
    observed = ssrf_proof.tool_sequence(messages)
    return [tool for tool in ssrf_proof.EXPECTED_TOOLS if tool not in observed]
...
def stop_stalled_app_stream() -> None:
    ...
    real_qwen.wait_until(
        ...
        "app stream stop after stalled SSRF exact-tool turn",
        timeout=15.0,
    )
...
def wait_for_final_marker(base_url: str, marker: str, timeout: float = 360.0) -> list[dict[str, Any]]:
    ...
    stall_seconds = float(os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_STALL_SECONDS", "45"))
    while time.time() < deadline:
        try:
            ...
            time.sleep(0.5)
            continue
        if ssrf_proof.has_assistant_marker(messages, marker) and not state.get("isWorking") and not state.get("isStreaming"):
            return messages
        ...
        if not state.get("isWorking") and not state.get("isStreaming"):
            sequence = ssrf_proof.tool_sequence(messages)
            ...
            if missing_expected_tools(messages) and assistant_messages and last_is_assistant:
                raise real_web.StalledFinalAnswer(messages, state, "assistant stopped before all expected SSRF/file-read tools")
            if ssrf_proof.ordered_subsequence(sequence, ssrf_proof.EXPECTED_TOOLS) and assistant_messages and last_is_assistant:
                if not last_assistant_text:
                    raise real_web.EmptyFinalAnswer(messages, state, "all SSRF/file-read tools ran, but the post-tool assistant answer was empty")
                raise real_web.MissingFinalMarker(messages, state, f"assistant answered after SSRF/file-read tools but omitted {marker}")
...
def synthesize_model_requests_from_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not schema_names:
        schema_names = list(ssrf_proof.EXPECTED_TOOLS)
    return [
...
def run() -> None:
    model = Path(os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_MODEL", str(MODEL_27B))).expanduser()
    output = output_path_for_model(model)
    fixture_module = ssrf_proof.load_fixture_module()
    fixture_session = fixture_module.build_fixture_session()
    lab_url = fixture_session.target_for("webserver_ssrf_file_read_chain")
    report: dict[str, Any] = {
        "ok": False,
        "proofType": "real-qwen-webserver-ssrf-fileread",
    ...
    app_home = tempfile.TemporaryDirectory(prefix="exploitbot-real-qwen-web-ssrf-home-", ignore_cleanup_errors=True)
    cache_tmp = tempfile.TemporaryDirectory(prefix="exploitbot-real-qwen-web-ssrf-cache-", ignore_cleanup_errors=True)
    ssrf_proof.FINAL_MARKER = FINAL_MARKER
    try:
        ...
        tools_dir = ssrf_proof.install_fake_ssrf_tools_at(home / ".exploitbot" / "tools", lab_url)
        env = os.environ.copy()
        ...
        with app_proof_lock("real-qwen-webserver-ssrf-fileread-proof.py"):
            subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            real_qwen.build_app_bundle()
            ssrf_proof.install_fake_ssrf_tools_at(APP_BINARY.parents[1] / "Resources" / "tools", lab_url)
            app = subprocess.Popen([str(APP_BINARY)], cwd=ROOT, env=env)
            wait_for_app()

            port = int(os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_ENGINE_PORT") or real_qwen.free_port())
            base_url = f"http://127.0.0.1:{port}"
            report["baseUrl"] = base_url
            report["modelLoadAttempted"] = True
            os.environ.setdefault(
                "EXPLOITBOT_REAL_QWEN_REAL_TOOLS_MAX_TOKENS",
                os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_MAX_TOKENS", "256"),
            )
            ...
            app_request(
                ...
                "/qa/apply-app-settings",
                {
                    ...
                    "engine": {
                        ...
                        "maxTokens": int(os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_MAX_TOKENS", "256")),
                        "temperature": 0.0,
                        "topP": 1.0,
            ...
            prompt = real_ssrf_prompt(lab_url)
            catalog = app_request(
                "POST",
                "/qa/tool-catalog",
            ...
            for tool in ssrf_proof.EXPECTED_TOOLS:
                require(tool in (catalog.get("toolNames") or []), f"tool schema missing before real Qwen SSRF turn: {tool}", catalog)
            report["preflightToolCatalog"] = catalog

            messages: list[dict[str, Any]] | None = None
            max_final_attempts = int(os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_FINAL_ATTEMPTS", "4"))
            for attempt in range(1, max_final_attempts + 1):
                ...
                try:
                    ...
                    report["finalAnswerAttempts"].append(
                        {
                            ...
                            "note": exc.note,
                            "toolSequence": ssrf_proof.tool_sequence(exc.messages),
                    ...
                    if attempt == max_final_attempts:
                        raise
            require(messages is not None, "real Qwen SSRF/file-read final answer missing after attempts", report["finalAnswerAttempts"])
            ...
            second_turn_attempts = int(os.environ.get("EXPLOITBOT_REAL_QWEN_WEBAPP_SSRF_SECOND_TURN_ATTEMPTS", "3"))
            for second_attempt in range(1, second_turn_attempts + 1):
                ...
                try:
                    ...
                    require(
                        ssrf_proof.tool_sequence(exc.messages) == ssrf_proof.EXPECTED_TOOLS,
                        "second model finalization retry must suppress parser tool calls without adding duplicate tools",
                        ssrf_proof.tool_sequence(exc.messages),
                    )
                    ...
                    if second_attempt == second_turn_attempts:
                        raise
            require(
                ssrf_proof.tool_sequence(messages) == ssrf_proof.EXPECTED_TOOLS,
                "second model turn must not add duplicate SSRF/file-read tools",
                ssrf_proof.tool_sequence(messages),
            )

            state = app_request("GET", "/state", timeout=10.0)
            results = app_request("GET", "/results", timeout=10.0)
            ssrf_proof.submit_report_from_results(lab_url)
            report_state = app_request("GET", "/state", timeout=10.0)
            cache_after = request_json("GET", f"{base_url}/v1/cache/stats", timeout=15.0)

            scenario_report = ssrf_proof.build_report(
            ...
            require(cache_status.get("multiTurnModelLoop") == "PASS", "real Qwen SSRF proof must process at least two model turns", cache_status)
            ...
            status = {
                ...
                "realQwenDroveWebserverSSRFFileRead": "PASS" if scenario_report.get("ok") is True else "FAIL",
                "ssrfProof": (scenario_report.get("checks") or {}).get("ssrfProof", "FAIL"),
                "fileReadProof": (scenario_report.get("checks") or {}).get("fileReadProof", "FAIL"),
            ...
            report.update(
                {
                    ...
                    "status": status,
                    "scenarioId": "real_qwen_webserver_ssrf_fileread",
                    "generatedAt": timestamp(),
                    "finishedAt": timestamp(),
                    "stages": ssrf_proof.STAGES,
                    "toolSequence": ssrf_proof.tool_sequence(messages),
                    "expectedToolSequence": ssrf_proof.EXPECTED_TOOLS,
                    "health": health,
                    ...
                    "notes": [
                        ...
                        "The SSRF and file-read validation uses harmless local fixture canaries only; it does not access cloud metadata or host-sensitive files.",
                    ],
                }
            )
            if not report["ok"]:
                raise AssertionError("real-Qwen SSRF/file-read checks failed", status)
    except Exception as exc:
        error = exc
    ...
    if error is not None:
        raise error
    print(f"real-qwen webserver SSRF/file-read proof passed: {output}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"real-qwen webserver SSRF/file-read proof failed: {exc}", flush=True)
        raise SystemExit(1)
