...
FINAL_MARKER = "REAL_QWEN_CONTAINER_IAC_SUPPLY_CHAIN_FINAL"
SECOND_TURN_MARKER = "REAL_QWEN_CONTAINER_IAC_SUPPLY_CHAIN_SECOND_TURN"
DEFAULT_OUTPUT_27B = ROOT / "docs/live-proofs/2026-07-06-real-qwen-container-iac-supply-chain-27b.json"
DEFAULT_OUTPUT_35B = ROOT / "docs/live-proofs/2026-07-06-real-qwen-container-iac-supply-chain-35b.json"
...
real_web = load_module(
    "exploitbot_real_qwen_webserver_auth_sqli_helpers_for_container_iac",
    ROOT / "scripts" / "real-qwen-webserver-auth-sqli-proof.py",
)
container_proof = real_web.load_module(
    "exploitbot_container_iac_real_qwen_contract",
    ROOT / "scripts" / "container-iac-supply-chain-scenario-proof.py",
)
...
def output_path_for_model(model: Path) -> Path:
    override = os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_OUTPUT")
    if override:
...
def container_tool_calls(fixture_path: str, tool_names: list[str] | None = None) -> list[tuple[str, dict[str, Any]]]:
    surface_cmd = (
    ...
    call_args: dict[str, dict[str, Any]] = {
        "run_shell": {"command": surface_cmd},
        "syft": {"target": fixture_path, "output": "json"},
        "grype": {"target": fixture_path, "fail_on": "high"},
        "trivy": {"target": fixture_path, "scan_type": "fs", "severity": "HIGH,CRITICAL"},
        "checkov": {"target": fixture_path, "framework": "kubernetes"},
        "search_cve": {"query": "CVE-2019-20372 nginx 1.16 container", "tags": "container,nginx,cve", "max_results": 5},
    }
    names = tool_names or list(container_proof.EXPECTED_TOOLS)
...
def real_container_prompt(fixture_path: str) -> str:
    return (
        "Authorized local container/IaC lab only. Reasoning is off. "
        f"Use only this throwaway local fixture path and no external targets or registries: {fixture_path}. "
        "Surface Dockerfile and Kubernetes files, run SBOM/vulnerability/IaC scanners, prove nginx:1.16, "
        "CVE-2019-20372, CKV_K8S_20, EXPLOITBOT_CONTAINER_IAC_PROOF, and allowPrivilegeEscalation: true, "
        "then summarize evidence for the report. Emit these exact Qwen XML tool calls now, with valid JSON. "
        f"After tool results, answer with marker {FINAL_MARKER} and summarize run_shell, syft, grype, trivy, "
        "checkov, search_cve, CVE-2019-20372, CKV_K8S_20, allowPrivilegeEscalation: true, and generated report evidence.\n"
        f"{serialize_tool_calls(container_tool_calls(fixture_path))}"
    )


def continue_remaining_tool_prompt(fixture_path: str, missing_tools: list[str]) -> str:
    return (
        "The previous assistant stream stalled before the local container/IaC workflow finished. "
        ...
        f"{serialize_tool_calls(container_tool_calls(fixture_path, missing_tools))}"
    )


def final_followup_prompt(fixture_path: str) -> str:
    return (
        "The local container/IaC scanner results are already in the transcript. "
        "Do not call more tools. Start the first line with this exact marker and then give a concise evidence summary: "
        f"{FINAL_MARKER}. Mention {fixture_path}, run_shell, syft, grype, trivy, checkov, search_cve, "
        "EXPLOITBOT_CONTAINER_IAC_PROOF, CVE-2019-20372, CKV_K8S_20, allowPrivilegeEscalation: true, and report evidence."
    )
...
def second_turn_prompt(attempt: int = 1) -> str:
    ...
    return (
        ...
        f"{SECOND_TURN_MARKER}. Mention EXPLOITBOT_CONTAINER_IAC_PROOF, CVE-2019-20372, CKV_K8S_20, "
        "allowPrivilegeEscalation: true, trivyEvidence, checkovEvidence, and local container/IaC evidence."
    )


def missing_expected_tools(messages: list[dict[str, Any]]) -> list[str]:
    observed = container_proof.tool_sequence(messages)
    return [tool for tool in container_proof.EXPECTED_TOOLS if tool not in observed]
...
def stop_stalled_app_stream() -> None:
    ...
    real_qwen.wait_until(
        ...
        "app stream stop after stalled container exact-tool turn",
        timeout=15.0,
    )
...
def wait_for_final_marker(base_url: str, marker: str, timeout: float = 360.0) -> list[dict[str, Any]]:
    ...
    stall_seconds = float(os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_STALL_SECONDS", "45"))
    while time.time() < deadline:
        try:
            ...
            time.sleep(0.5)
            continue
        if container_proof.has_assistant_marker(messages, marker) and not state.get("isWorking") and not state.get("isStreaming"):
            return messages
        ...
        if not state.get("isWorking") and not state.get("isStreaming"):
            sequence = container_proof.tool_sequence(messages)
            ...
            if missing_expected_tools(messages) and assistant_messages and last_is_assistant:
                raise real_web.StalledFinalAnswer(messages, state, "assistant stopped before all expected container/IaC tools")
            if container_proof.ordered_subsequence(sequence, container_proof.EXPECTED_TOOLS) and assistant_messages and last_is_assistant:
                if not last_assistant_text:
                    raise real_web.EmptyFinalAnswer(messages, state, "all container/IaC tools ran, but the post-tool assistant answer was empty")
                raise real_web.MissingFinalMarker(messages, state, f"assistant answered after container/IaC tools but omitted {marker}")
...
def synthesize_model_requests_from_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not schema_names:
        schema_names = list(container_proof.EXPECTED_TOOLS)
    return [
...
def run() -> None:
    model = Path(os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_MODEL", str(MODEL_27B))).expanduser()
    output = output_path_for_model(model)
    fixture_module = container_proof.load_fixture_module()
    ...
    report: dict[str, Any] = {
        "ok": False,
        "proofType": "real-qwen-container-iac-supply-chain",
        "proofLevel": "live-app-real-qwen-real-tool-loop-local-container-iac-fixture",
        "model": str(model),
    ...
    app_home = tempfile.TemporaryDirectory(prefix="exploitbot-real-qwen-container-iac-home-", ignore_cleanup_errors=True)
    cache_tmp = tempfile.TemporaryDirectory(prefix="exploitbot-real-qwen-container-iac-cache-", ignore_cleanup_errors=True)
    container_proof.FINAL_MARKER = FINAL_MARKER
    try:
        ...
        tools_dir = container_proof.install_fake_container_tools_at(home / ".exploitbot" / "tools")
        env = os.environ.copy()
        ...
        with app_proof_lock("real-qwen-container-iac-supply-chain-proof.py"):
            subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            real_qwen.build_app_bundle()
            container_proof.install_fake_container_tools_at(APP_BINARY.parents[1] / "Resources" / "tools")
            app = subprocess.Popen([str(APP_BINARY)], cwd=ROOT, env=env)
            wait_for_app()

            port = int(os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_ENGINE_PORT") or real_qwen.free_port())
            base_url = f"http://127.0.0.1:{port}"
            report["baseUrl"] = base_url
            report["modelLoadAttempted"] = True
            os.environ.setdefault(
                "EXPLOITBOT_REAL_QWEN_REAL_TOOLS_MAX_TOKENS",
                os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_MAX_TOKENS", "256"),
            )
            ...
            app_request(
                ...
                "/qa/apply-app-settings",
                {
                    ...
                    "engine": {
                        ...
                        "maxTokens": int(os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_MAX_TOKENS", "256")),
                        "temperature": 0.0,
                        "topP": 1.0,
            ...
            prompt = real_container_prompt(fixture_path)
            catalog = app_request(
                "POST",
                "/qa/tool-catalog",
            ...
            for tool in container_proof.EXPECTED_TOOLS:
                require(tool in (catalog.get("toolNames") or []), f"tool schema missing before real Qwen container/IaC turn: {tool}", catalog)
            report["preflightToolCatalog"] = catalog

            messages: list[dict[str, Any]] | None = None
            max_final_attempts = int(os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_FINAL_ATTEMPTS", "4"))
            for attempt in range(1, max_final_attempts + 1):
                if attempt == 1:
                    ...
                    app_request("POST", "/send", continue_remaining_tool_prompt(fixture_path, missing_tools), timeout=15.0)
                else:
                    app_request(
                        "POST",
                        "/qa/apply-app-settings",
                    ...
                    app_request("POST", "/send", final_followup_prompt(fixture_path), timeout=15.0)
                try:
                    ...
                    report["finalAnswerAttempts"].append(
                        {
                            ...
                            "note": exc.note,
                            "toolSequence": container_proof.tool_sequence(exc.messages),
                    ...
                    if attempt == max_final_attempts:
                        raise
            require(messages is not None, "real Qwen container/IaC final answer missing after attempts", report["finalAnswerAttempts"])
            ...
            second_turn_attempts = int(os.environ.get("EXPLOITBOT_REAL_QWEN_CONTAINER_IAC_SECOND_TURN_ATTEMPTS", "3"))
            for second_attempt in range(1, second_turn_attempts + 1):
                ...
                try:
                    ...
                    require(
                        container_proof.tool_sequence(exc.messages) == container_proof.EXPECTED_TOOLS,
                        "second model finalization retry must suppress parser tool calls without adding duplicate tools",
                        container_proof.tool_sequence(exc.messages),
                    )
                    ...
                    if second_attempt == second_turn_attempts:
                        raise
            require(
                container_proof.tool_sequence(messages) == container_proof.EXPECTED_TOOLS,
                "second model turn must not add duplicate container/IaC tools",
                container_proof.tool_sequence(messages),
            )

            state = app_request("GET", "/state", timeout=10.0)
            results = app_request("GET", "/results", timeout=10.0)
            container_proof.submit_report_from_results(fixture_path)
            report_state = app_request("GET", "/state", timeout=10.0)
            cache_after = request_json("GET", f"{base_url}/v1/cache/stats", timeout=15.0)

            scenario_report = container_proof.build_report(
            ...
            require(cache_status.get("multiTurnModelLoop") == "PASS", "real Qwen container/IaC proof must process at least two model turns", cache_status)
            ...
            status = {
                ...
                "realQwenDroveContainerIac": "PASS" if scenario_report.get("ok") is True else "FAIL",
                "trivyEvidence": (scenario_report.get("checks") or {}).get("trivyEvidence", "FAIL"),
                "checkovEvidence": (scenario_report.get("checks") or {}).get("checkovEvidence", "FAIL"),
            ...
            report.update(
                {
                    ...
                    "status": status,
                    "scenarioId": "real_qwen_container_iac_supply_chain",
                    "generatedAt": timestamp(),
                    "finishedAt": timestamp(),
                    "stages": container_proof.STAGES,
                    "toolSequence": container_proof.tool_sequence(messages),
                    "expectedToolSequence": container_proof.EXPECTED_TOOLS,
                    "health": health,
                    ...
                    "notes": [
                        "This proof uses a throwaway local container/IaC fixture and deterministic local syft/grype/trivy/checkov/search_cve tool shims on isolated PATH.",
            ...
            if not report["ok"]:
                raise AssertionError("real-Qwen container/IaC checks failed", status)
    except Exception as exc:
        error = exc
    ...
    if error is not None:
        raise error
    print(f"real-qwen container/IaC supply-chain proof passed: {output}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"real-qwen container/IaC supply-chain proof failed: {exc}", flush=True)
        raise SystemExit(1)
