...
repo_proof = load_module("exploitbot_repo_supply_chain_proof", ROOT / "scripts" / "repo-codebase-supply-chain-scenario-proof.py")
...
def output_path_for_model(model: Path) -> Path:
    override = os.environ.get("EXPLOITBOT_REAL_QWEN_REPO_OUTPUT")
    if override:
...
def real_repo_prompt(repo: Path) -> str:
    calls = [
    ...
    return (
        "Authorized local repo/codebase supply-chain proof. Reasoning is off. "
        "Use only this throwaway local repo path and do not touch external targets: "
        f"{repo}. Surface files, probe secrets/SBOM/dependencies, prove known vulnerable packages, "
        "validate safely without exploitation, then summarize evidence for the report. "
        "Emit these exact Qwen XML tool calls now, with valid JSON. "
        f"After tool results, answer with marker {FINAL_MARKER} and summarize the captured tool evidence.\n"
        f"{serialized}"
    )
...
def wait_for_final_marker(base_url: str, marker: str, timeout: float = 360.0) -> list[dict[str, Any]]:
    ...
    stall_seconds = float(os.environ.get("EXPLOITBOT_REAL_QWEN_REPO_STALL_SECONDS", "45"))
    while time.time() < deadline:
        try:
            ...
            time.sleep(0.5)
            continue
        if repo_proof.has_assistant_marker(messages, marker) and not state.get("isWorking") and not state.get("isStreaming"):
            return messages
        ...
        if not state.get("isWorking") and not state.get("isStreaming"):
            sequence = repo_proof.tool_sequence(messages)
            ...
            if (
                ...
                raise EmptyFinalAnswer(messages, state, "all repo tools ran, but the post-tool assistant answer was empty")
            if (
                repo_proof.ordered_subsequence(sequence, repo_proof.EXPECTED_TOOLS)
                and assistant_messages
                and last_is_assistant
                ...
                raise MissingFinalMarker(messages, state, f"assistant answered after repo tools but omitted {marker}")
...
def synthesize_model_requests_from_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not schema_names:
        schema_names = list(repo_proof.EXPECTED_TOOLS)
    return [
...
def submit_report_from_results(repo: Path, results: dict[str, Any]) -> dict[str, Any]:
    grype_vuln = next(
        (row for row in results.get("vulns") or [] if isinstance(row, dict) and row.get("source") == "grype"),
        {},
    )
    osv_vuln = next(
        (row for row in results.get("vulns") or [] if isinstance(row, dict) and row.get("source") == "osv_scanner"),
        {},
    )
    created = app_request(
        "POST",
    ...
    require(created.get("ok") is True, "report finding create failed", created)
    generated = app_request("POST", "/qa/report-generate-action", {"template": "Full Report"}, timeout=15.0)
    require(generated.get("ok") is True, "report generation failed", generated)
    return generated
...
def final_followup_prompt() -> str:
    return (
        "The local repo/codebase tool results are already in the transcript. "
...
def run() -> None:
    model = Path(os.environ.get("EXPLOITBOT_REAL_QWEN_REPO_MODEL", str(MODEL_27B))).expanduser()
    output = output_path_for_model(model)
    report: dict[str, Any] = {
        "ok": False,
        "proofType": "real-qwen-repo-codebase-supply-chain",
        "proofLevel": "live-app-real-qwen-real-tool-loop-local-repo-fixture",
        "model": str(model),
        "startedAt": timestamp(),
    ...
    app_home = tempfile.TemporaryDirectory(prefix="exploitbot-real-qwen-repo-home-", ignore_cleanup_errors=True)
    repo_tmp = tempfile.TemporaryDirectory(prefix="exploitbot-real-qwen-repo-fixture-", ignore_cleanup_errors=True)
    cache_tmp = tempfile.TemporaryDirectory(prefix="exploitbot-real-qwen-repo-cache-", ignore_cleanup_errors=True)
    repo_proof.FINAL_MARKER = FINAL_MARKER
    try:
        ...
        repo = repo_proof.create_repo_fixture(Path(repo_tmp.name))
        tools_dir = repo_proof.install_fake_tools(home)
        env = os.environ.copy()
        ...
        with app_proof_lock("real-qwen-repo-codebase-supply-chain-proof.py"):
            subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            real_qwen.build_app_bundle()
            repo_proof.install_fake_tools_at(APP_BINARY.parents[1] / "Resources" / "tools")
            app = subprocess.Popen([str(APP_BINARY)], cwd=ROOT, env=env)
            wait_for_app()

            port = int(os.environ.get("EXPLOITBOT_REAL_QWEN_REPO_ENGINE_PORT") or real_qwen.free_port())
            base_url = f"http://127.0.0.1:{port}"
            report["baseUrl"] = base_url
            report["modelLoadAttempted"] = True
            os.environ.setdefault(
                "EXPLOITBOT_REAL_QWEN_REAL_TOOLS_MAX_TOKENS",
                os.environ.get("EXPLOITBOT_REAL_QWEN_REPO_MAX_TOKENS", "256"),
            )
            ...
            app_request(
                ...
                "/qa/apply-app-settings",
                {
                    ...
                    "engine": {
                        ...
                        "maxTokens": int(os.environ.get("EXPLOITBOT_REAL_QWEN_REPO_MAX_TOKENS", "256")),
                        "temperature": 0.0,
                        "topP": 1.0,
            ...
            for tool in repo_proof.EXPECTED_TOOLS:
                require(tool in (catalog.get("toolNames") or []), f"tool schema missing before real Qwen repo turn: {tool}", catalog)
            ...
            scenario_report = repo_proof.build_report(
            ...
            status = {
                ...
                "realQwenDroveRepoSupplyChain": "PASS" if scenario_report.get("ok") is True else "FAIL",
                "dependencyStructuredFindings": (scenario_report.get("checks") or {}).get("dependencyStructuredFindings", "FAIL"),
            ...
            report.update(
                {
                    ...
                    "status": status,
                    "scenarioId": "real_qwen_repo_codebase_supply_chain",
                    "generatedAt": timestamp(),
                    "finishedAt": timestamp(),
                    "repoPath": str(repo),
                    "stages": repo_proof.STAGES,
                    "toolSequence": repo_proof.tool_sequence(messages),
                    "expectedToolSequence": repo_proof.EXPECTED_TOOLS,
                    "health": health,
                    ...
                    "notes": [
                        "This proof uses a throwaway local repo and fake local scanner binaries on isolated PATH.",
                        "The model endpoint is a real Qwen app engine route; the repo scanners are local fixtures for deterministic safety.",
                        ...
                        "Default repo proof maxTokens is intentionally bounded to keep retry/stall behavior observable without flooding RAM.",
                    ],
                }
            )
            if not report["ok"]:
                raise AssertionError("real-Qwen repo-codebase supply-chain checks failed", status)
    except Exception as exc:
        error = exc
    ...
    if error is not None:
        raise error
    print(f"real-qwen repo-codebase supply-chain proof passed: {output}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"real-qwen repo-codebase supply-chain proof failed: {exc}", flush=True)
        raise SystemExit(1)
