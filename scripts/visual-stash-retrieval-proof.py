...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-92"
...
def run() -> None:
    ...
    try:
        ...
        request("POST", "/qa/context-packet", {
        ...
        request("POST", "/tab", "stash")

        state = request("GET", "/state")
        retrieval = state.get("stashRetrieval") or {}
        if (retrieval.get("topLabels") or [])[:1] != ["kerberos-golden-ticket-note"]:
            raise AssertionError(f"stash retrieval visual state missing targeted top label: {retrieval}")

        target = OUT_DIR / "stash-retrieval-audit.png"
        capture(target)
        manifest = {
            "ok": True,
            "captures": [str(target.relative_to(ROOT))],
            "note": "Cropped macOS capture of Stash tab showing scored retrieval audit after model-context catalogue selection.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-stash-retrieval proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-stash-retrieval proof failed: {exc}", flush=True)
        raise SystemExit(1)
