...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-98"
...
def run() -> None:
    ...
    try:
        ...
        if state.get("activeTab") != "network" or (state.get("networkAction") or {}).get("status") != "running":
            raise AssertionError(f"expected running network action state: {state}")
        target = OUT_DIR / "network-protocol-running.png"
        capture(target)
        manifest = {
            "ok": True,
            "captures": [str(target.relative_to(ROOT))],
            "note": "Cropped macOS capture of Network Protocols tab showing Scan running button state and tab activity indicator.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-network-protocol proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-network-protocol proof failed: {exc}", flush=True)
        raise SystemExit(1)
