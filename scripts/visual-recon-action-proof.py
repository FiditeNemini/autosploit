...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-97"
...
def run() -> None:
    ...
    try:
        ...
        if state.get("activeTab") != "recon" or (state.get("reconAction") or {}).get("status") != "running":
            raise AssertionError(f"expected running recon action state: {state}")
        target = OUT_DIR / "recon-full-running.png"
        capture(target)
        manifest = {
            "ok": True,
            "captures": [str(target.relative_to(ROOT))],
            "note": "Cropped macOS capture of Recon tab showing Full Recon running button state and tab activity indicator.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-recon-action proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-recon-action proof failed: {exc}", flush=True)
        raise SystemExit(1)
