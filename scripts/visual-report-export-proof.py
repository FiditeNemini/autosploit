...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-91"
...
def assert_state() -> None:
    ...
    if state.get("activeTab") != "report":
        raise AssertionError(f"expected Report tab: {state}")
    report = state.get("reportExport") or {}
    if report.get("status") != "done" or report.get("findingCount") != 1:
        raise AssertionError(f"expected completed report export state: {report}")
    if len(report.get("artifacts") or []) != 4:
        raise AssertionError(f"expected four report artifacts: {report}")
...
def run() -> None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="exploitbot-report-visual-") as tmp:
            seeded = request("POST", "/qa/seed-report-export", {"outputDir": tmp})
            if seeded.get("ok") is not True:
                raise AssertionError(f"seed failed: {seeded}")
            assert_state()
            target = OUT_DIR / "report-export-status.png"
            capture(target)
            manifest = {
                "ok": True,
                "captures": [str(target.relative_to(ROOT))],
                "note": "Cropped macOS capture of Report tab showing generated finding plus export status strip after deterministic artifact export.",
            }
            (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-report-export proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-report-export proof failed: {exc}", flush=True)
        raise SystemExit(1)
