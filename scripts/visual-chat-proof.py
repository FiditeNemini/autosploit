...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-71"
...
def assert_seeded_chat_state() -> None:
    state = request("GET", "/state")
    if state.get("msgs", 0) < 6:
        raise AssertionError(f"expected seeded chat messages: {state}")
    chat = state.get("chat") or {}
    if chat.get("enableReasoning") is not True:
        raise AssertionError(f"expected reasoning on in seeded chat state: {state}")
    ...
    required = {
        ("approval", "run_shell", "pending"),
        ("toolCall", "run_shell", "running..."),
        ("toolCall", "hashcat", "fail 1.2s"),
        ("thinking", "", ""),
    }
    seen = set()
    for msg in messages:
        seen.add((msg.get("role", ""), msg.get("tool", ""), msg.get("status", "")))
    missing = required - seen
    if missing:
        raise AssertionError(f"seeded chat messages missing required states {missing}: {messages}")
...
def run() -> None:
    ...
    try:
        ...
        assert_seeded_chat_state()

        target = OUT_DIR / "chat-tool-states.png"
        capture(target)
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-chat proof failed: {exc}", flush=True)
        raise SystemExit(1)
