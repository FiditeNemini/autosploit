...
OUT_DIR = ROOT / "docs" / "visual-proofs" / "checkpoint-94"
...
def run() -> None:
    ...
    try:
        ...
        if state.get("activeTab") != "post" or len(state.get("postAttribution") or []) < 3:
            raise AssertionError(f"expected post attribution state: {state}")
        target = OUT_DIR / "post-attribution.png"
        capture(target)
        manifest = {
            "ok": True,
            "captures": [str(target.relative_to(ROOT))],
            "note": "Cropped macOS capture of Post tab showing output attribution rows for linpeas host, impacket secretsdump target, and metasploit session host.",
        }
        (OUT_DIR / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print("visual-post-attribution proof passed")
    finally:
        subprocess.run(["pkill", "-x", "ExploitBot"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if app.poll() is None:
...
if __name__ == "__main__":
    try:
        ...
        print(f"visual-post-attribution proof failed: {exc}", flush=True)
        raise SystemExit(1)
